    ...
    def solve_after(self, city):
        for i, c in enumerate(city):
            if c.count(0) is not 0:
                match = list(filter(lambda r: ((r[0] == c[0] or c[0] is 0) and (r[1] == c[1] or c[1] is 0) and (r[2] == c[2] or c[2] is 0) and (r[3] == c[3] or c[3] is 0) and (r[4] == c[4] or c[4] is 0) and (r[5] == c[5] or c[5] is 0)), itertools.permutations([1, 2, 3, 4, 5, 6])))
                for m in match:
